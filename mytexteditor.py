...
Label(top_frame,text="Font styles").pack(side=LEFT,padx=5)
style_var = StringVar(value="italic")
def change_font_style(*args):
    style = style_var.get()
...
def show_word_count(event = None):
    ...
    messagebox.showinfo("Word count",f"words:{len(word)}\n characters:{characters}")
def change_font_size():
    size = simpledialog.askinteger("Font size","Enter font size",minvalue=8,maxvalue=80)
    if size:
        text_font.config(size=size)
def find_replace(event = None):
    find = simpledialog.askstring("Find","Enter text to find")
    replace = simpledialog.askstring("Replace","Replace text")
